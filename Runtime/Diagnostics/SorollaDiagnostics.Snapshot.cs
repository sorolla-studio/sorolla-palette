    ...
    internal static partial class SorollaDiagnostics
    {
        static Snapshot CaptureSnapshot()
        {
            lock (s_lock)
            {
                return new Snapshot
                {
                    ...
                    GaInitialized = AdapterOutcomeIsReady(s_gameAnalyticsOutcome),
                    GameAnalyticsOutcome = s_gameAnalyticsOutcome,
                    FacebookInitialized = AdapterOutcomeIsReady(s_facebookOutcome),
                    FacebookFailed = s_facebookOutcome.Status == AdapterDiagnosticStatus.Failed,
                    ...
                    MaxRegistered = MaxAdapter.IsRegistered,
                    MaxInitialized = MaxAdapter.IsInitialized,
                    MaxOutcome = s_maxAdapterOutcome,
                    MaxConsentSeen = s_maxConsentSeen,
                    MaxConsentDetail = s_maxConsentDetail,
                    AdjustRegistered = AdjustAdapter.IsRegistered,
                    AdjustInitializing = s_adjustInitializing,
                    AdjustInitialized = AdjustAdapter.IsInitialized,
                    AdjustMissingToken = s_adjustMissingToken,
                    AdjustEnvironment = s_adjustEnvironment,
                    AdjustOutcome = s_adjustAdapterOutcome,
                    FirebaseCoreReady = AdapterOutcomeIsReady(s_firebaseCoreOutcome),
                    FirebaseAnalyticsReady = AdapterOutcomeIsReady(s_firebaseAnalyticsOutcome),
                    CrashlyticsReady = AdapterOutcomeIsReady(s_crashlyticsOutcome),
                    ...
                    RemoteConfigFetchSeen = RemoteConfigFetchSeen(),
                    RemoteConfigFetchSuccess = s_remoteConfigOutcome.Code == "fetch_complete",
                    RemoteConfigDetail = RemoteConfigDetail(),
                    ...
                    PurchaseVerificationState = s_purchaseVerificationState,
        ...
        internal static SorollaQaState CaptureQaState()
        {
            ...
            return new SorollaQaState
            {
                ...
                IapVerificationState = snap.PurchaseVerificationState,
