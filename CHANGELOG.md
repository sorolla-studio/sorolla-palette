...
- **Trimmed dead adapter API, obsolete IAP-v5 shims, and duplicated editor/sanitizer logic** (`Runtime/Adapters/*`, `Runtime/Purchasing/*`, `Editor/*`). Removed unused adapter methods, the `AutoTracker` purchase shim, and obsolete IAP-v5 compatibility code with no remaining consumer; deduplicated the Android manifest / gradle sanitizers and editor define logic. No public `Palette` API change.
