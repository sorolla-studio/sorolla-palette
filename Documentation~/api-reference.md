...
where no on-device collector exists yet (they resolve INCOMPLETE, a capability gap). `None`
...
store console). Distinct from `distributionPlatforms`: a game can ship its app on
...
`None` is undeclared and fails the store gate closed to INCOMPLETE.
...
## Enum SorollaPlatforms
A set of store platforms - a curated flags value, not loose per-platform booleans. Used for two
distinct declarations on `SorollaConfig`: DISTRIBUTION (where the game ships, drives the
device/QA gates) and COMMERCE (where it sells in-app purchases, drives the store-config gate).
`None` means undeclared - the greenlight fails the dependent gates closed to INCOMPLETE until a
