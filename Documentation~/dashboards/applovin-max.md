...
This is **ruled not probe-coverable**: no API call the SDK can make from the client reaches into MAX's or Adjust's server-side FAN/attribution config to check what Facebook App ID they have on file. It is therefore a dashboard check you do here, not a row in the SDK window.

## What the verdict shows when it's wrong

| Row | State | Meaning |
|---|---|---|
| _(no window row)_ | Nothing — this is dashboard work | Confirm the Facebook App ID referenced in MAX's FAN mediation setup and in Adjust's Facebook integration setting both match the live, current Facebook app (see [facebook.md](facebook.md) for confirming the FB app itself is live). The runtime capture does catch the downstream symptom: a Facebook OAuth rejection shows up as an SDK error on device. |

There is no automated signal for this drift today; a future runtime MAX adapter-error listener (not yet built) is the only planned path to catching it mechanically. Until then it is a genuinely manual dashboard-to-dashboard comparison — done here, in the dashboards, because a checkbox in an editor window would have recorded an opinion rather than evidence.
