...
The Launch Readiness status reads `HEALTHY`, `N ISSUES`, `INCOMPLETE`, or `FAILING`. It is an interim integration self-check, not a release verdict. Connected-device Vitals is displayed separately and does not vote in Launch Readiness. Vitals answers what happened in the running build and renders `FAILING`, `N ISSUES`, `NOT PROVEN`, or `HEALTHY`.
...
**No password.** Neither endpoint requires auth. The loopback-only bind blocks direct LAN access, but device loopback is device-global: another app/process on the device can connect. A host computer needs USB forwarding. Treat anything returned by the bridge as potentially readable and don't send secrets to it.
